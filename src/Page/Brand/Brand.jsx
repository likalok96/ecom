...
import {getFirstLetter} from '../../utils/ProductFn/getFirstLetter'
 ...
 */
    ...
    let first_letter = getFirstLetter(filter_brand)
        ...
        return(
            <div className='shop_brand'>
                ...
                <div className='shop_brand_inner_wrapper'>
                    {brand.map((brand)=><a href={`/collection/brand/${brand}`}  key={brand}>{brand}</a>)}
                </div>
            </div>
        )
    }
// || !loading
    if(!filter_brand) return (
        <LoadingSpinner />
    )
    
  return (
    <div className='shop_brand_wrapper'>
        {first_letter.map((letter)=><BrandText text={letter} key={letter}/>)}
    </div>
  )
}
